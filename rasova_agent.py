...
def _watchdog_bat_path() -> str:
    return os.path.join(CONFIG_DIR, "RasovaAgentWatchdog.bat")
...
def install_autostart():
    ...
    bat_contents = (
        "@echo off\n"
        ":: Auto-generated by rasova_agent.py --install\n"
        ":: Restarts the Rasova Print Agent if it ever exits (crash, kill,\n"
...
def uninstall_autostart():
    ...
    if IS_WINDOWS:
        ...
        try:
            subprocess.run(
                ["wmic", "process", "where",
                 "CommandLine like '%RasovaAgentWatchdog.bat%'",
                 "call", "terminate"],
                capture_output=True, check=False, timeout=10,
            )
            subprocess.run(
                ["wmic", "process", "where",
                 "CommandLine like '%rasova_agent.py%'",
                 "call", "terminate"],
                capture_output=True, check=False, timeout=10,
            )
        except Exception:
